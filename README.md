...
```bash
./test_openai.sh --api-type chat "Что такое ИИ?"
```

**Тестировать только Responses API:**

```bash
./test_openai.sh --api-type responses "Расскажи про машинное обучение"
```
...
| `--api-type TYPE` | Тип API для тестирования (`chat`, `responses`, `both`) | `both`                              |
...
source custom.env
./test_openai.sh --api-type chat "Test message"
```
